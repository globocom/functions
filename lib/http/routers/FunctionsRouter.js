...
const { FunctionProviderFactory } = require('../../domain/FunctionProvider');
...
const multipartParser = (req) => {
  ...
  return new Promise((resolve, reject) => {
    form.parse(req, function(err, fields, files) {
      ...
      const fs = require("fs")
      // fs.readFile(files.path)
      resolve({fields, uploads: files})
    });
  })
}
...
router.put('/:namespace/:id', bodyParser.json({ limit: bodyParserLimit }), async (req, res, next) => {
  ...
  if(isExternalFunctionProvider){
    const {fields, uploads} = await multipartParser(req)
    req.body = JSON.parse(fields['body'])

    if(uploads && uploads.file){
      const [zip] = uploads.file
      req.file = zip
    }else{
      throw "Empty function source upload"
    }
  }
  ...
  const data = { id, code, hash, provider: functionProvider, runtime, entrypoint };
  if (env) {
    data.env = env;
  }
  ...
  try {
    ...
    const providerId = await memoryStorage.getProvider(namespace, id);
    const factory = new FunctionProviderFactory();
    const provider = factory.getFunctionProvider(providerId);

    const deployResult = await provider.deployFunction(req);
...
router.all('/:namespace/:id/run', bodyParser.json({ limit: bodyParserLimit }), async (req, res, next) => {
  ...
  try {
    ...
    const executorId = memoryStorage.getExecutorByCache(namespace, id);

    factory = new FunctionExecutorFactory(executorId);

    const executor = factory.getFunctionExecutor(executor);
