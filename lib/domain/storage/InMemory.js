...
function storageKey(namespace, codeId, version) {
  if (!version) {
...
function functionInMemoryKey(namespace, id, version) {
...
function getFunctionInMemoryKey(functionStorageKey) {
  const items = functionStorageKey.split(':', 3);
  return {
    namespace: items[0],
    id: items[1],
    version: items[2],
...
async function getMultiCodes(storage, codes, preCache) {
  const keys = codes.map(({ namespace, id, version }) => storageKey(namespace, id, version));
...
class StorageInMemory extends Storage {
  ...
  async putCode(namespace, id, version, code) {
    const key = storageKey(namespace, id, version);
  ...
  setNamespaceMember(namespace, id, version) {
    const key = functionInMemoryKey(namespace, id, version);
  ...
  async deleteCode(namespace, id, version) {
    const key = storageKey(namespace, id, version);
  ...
  async getCodeByCache(namespace, id, version, { preCache }) {
    const item = await this.getCode(namespace, id, version);
  ...
  async search(namespace, id, version, page = 1, perPage = 10) {
    ...
    const functionKey = functionInMemoryKey(namespace, id || '', version || '');
    const items = [];
