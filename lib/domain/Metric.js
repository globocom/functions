...
const functionRunScriptHistogram = new prometheusClient.Histogram({
  name: 'backstage_functions_run_script_duration_seconds',
  help: 'How many time have been spent in vm.Script.runInContext in seconds',
  buckets,
  labelNames: ['namespace', 'id'],
});
...
class Metric {
  ...
  observeFunctionRunScript({ namespace, id }) {
    const spent = (Date.now() - this.start) / 1000;
    functionRunScriptHistogram.labels(namespace, id).observe(spent);
    return spent;
  }
  ...
  observeFunctionSetup() {
    const spent = (Date.now() - this.start) / 1000;

    functionSetupHistogram.observe(spent);
    functionSetupCounter.inc();
