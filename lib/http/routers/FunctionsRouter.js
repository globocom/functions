...
router.post('/:namespace/:id', bodyParser.json(), (req, res) => {
  ...
  const sandboxLog = new SandboxLog(`${namespace}/${id}`);

  const invalid = sandbox.testSyntaxError(filename, code, sandboxLog);
  if (invalid) {
...
router.put('/:namespace/:id/run', bodyParser.json(), (req, res) => {
  ...
  memoryStorage
    ...
    .then((code) => {
      ...
      return sandbox.runScript(code.script, req, sandboxLog);
    })
    .then((result) => {
      ...
      log.error(`[${namespace}:${id}] Failed to run function`);
      log.error(`[${namespace}:${id}] ${err}`);
