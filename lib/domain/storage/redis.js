...
function redisKey(namespace, codeId, version) {
  if (!version) {
    return `code:${namespace}/${codeId}`;
  }
  return `code:${namespace}/${codeId}/${version}`;
}
...
function functionRedisKey(namespace, id, version) {
  if (!version) {
    return `${namespace}:${id}`;
  }
...
function getFunctionRedisKey(functionKey) {
  const items = functionKey.split(':', 3);
  return {
    namespace: items[0],
    id: items[1],
    version: items[2],
...
async function getCodeAndPopulateCache(storage, namespace, id, version, preCache) {
  log.info(`Get code from database, namespace: ${namespace} codeId: ${id} version: ${version} and store on cache`);

  const code = await storage.getCode(namespace, id, version);
  if (!code) {
    return null;
  }

  const cacheItem = preCache(code);
  const key = redisKey(namespace, id, version);
...
async function getMultiCodes(storage, codes, preCache) {
  const keys = codes.map(({ namespace, id, version }) => redisKey(namespace, id, version));
  ...
  return results.map(([err, versionID], i) => {
    ...
    const key = keys[i];
    const { namespace, id, version } = codes[i];
    ...
    // populate the cache
    return getCodeAndPopulateCache(storage, namespace, id, version, preCache);
  });
}
...
class StorageRedis extends Storage {
  ...
  async search(namespace, id, version, page = 1, perPage = 10) {
    const total = await this.client.zcount('namespaces', '-inf', '+inf');
    ...
    const minRange = `[${functionRedisKey(namespace, id || '*', version || '*')}`;
    const maxRange = `[${functionRedisKey(namespace, id || '\xff', version || '\xff')}`;
  ...
  setNamespaceMember(namespace, id, version) {
    return this.client.zadd('namespaces', 0, functionRedisKey(namespace, id, version));
  }

  deleteNamespaceMember(namespace, id, version) {
    return this.client.zrem('namespaces', 0, functionRedisKey(namespace, id, version));
  }
  ...
  async putCode(namespace, id, version, code) {
    ...
    this.setNamespaceMember(namespace, id, version);
    return this.client.hmset(key, data);
  }

  async getCode(namespace, id, version) {
    const key = redisKey(namespace, id, version);
  ...
  deleteCode(namespace, id, version) {
    const key = redisKey(namespace, id, version);

    this.deleteNamespaceMember(namespace, id, version);
  ...
  async getCodeByCache(namespace, id, version, { preCache }) {
    ...
    return getCodeAndPopulateCache(this, namespace, id, version, preCache);
  }
  ...
  async putCodeEnviromentVariable(namespace, id, version, env, value) {
    ...
    code.env[env] = value;
    return this.putCode(namespace, id, version, { env: code.env });
  }
  ...
  async getNamespace(namespace) {
    ...
    if (data.latest) {
      try {
        ...
        log.debug('Failed to deserialize latest info');
      }
    }
  ...
  putNamespace(namespace, data) {
    const key = namespaceRedisKey(namespace);
    if (data.latest) {
      const latest = JSON.stringify(data.latest);
      data.latest = latest;
    }
    return this.client.hmset(key, data);
  }
