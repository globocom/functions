...
class GelfLogStorage {
  ...
  static prepareGelfClients() {
    if (GelfLogStorage.gelfClientList) {
      return GelfLogStorage.gelfClientList;
    }

    let client;
    GelfLogStorage.gelfClientList = [];
    ...
    for (const host of config.log.hosts) {
      client = new Gelf({
        ...
        connection: 'wan',
        maxChunkSizeWan: config.log.maxChunkSize,
      });
      ...
      GelfLogStorage.gelfClientList.push(client);
    }
  ...
  flush({ status, requestTime }) {
    ...
    const extra = Object.assign({
      ...
      file: this.file,
    }, this.fields, config.log.fields);

    const message = Object.assign({
      full_message: fullMessage,
      level: this.logLevels[level],
      short_message: shortMessage,
    }, GelfLogStorage.prepareExtraFields(extra));
