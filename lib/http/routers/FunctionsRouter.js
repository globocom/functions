...
const { FunctionExecutorFactory } = require('../../domain/FunctionExecutor');
...
router.put('/:namespace/:id', bodyParser.json({ limit: bodyParserLimit }), async (req, res, next) => {
  ...
  const data = { id, code, hash, provider: functionProvider, projectId };
...
router.all('/:namespace/:id/run', bodyParser.json({ limit: bodyParserLimit }), async (req, res, next) => {
  ...
  try {
    const executorId = await memoryStorage.getExecutor(namespace, id);

    factory = new FunctionExecutorFactory();

    const executor = factory.getFunctionExecutor(executorId);
